Program harpn_newDRS_input2pams
  use Common
  use Fits
  use Instrument_HARPN_newDRS
  use CCF
  use Chebyshev_Fit
  use Poly_Fit
  use Gaussian_Fit
...
!!! write(*,*) cheb2_xord, cheb2_yord, cheb2_polyord
  ...
  do while (read_iostat.eq.0)
     ...
     if (check.eq.0) then
        ...
        file = trim(archive_files)//'/'//trim(file_night)//'/'//trim(file_rad)//'_S2D_'//trim(fiber_sel)//'.fits'
        ...
        call fits_open_hdu_s2d(fits_lun,file,2,d2pix)
        call fits_close(fits_lun)

        call fits_open_hdu_s2d(fits_lun,file,6,w2pix)
        call fits_close(fits_lun)

        call fits_open_hdu_s2d(fits_lun,file,8,dwpix)
        call fits_close(fits_lun)
        ...
        ! RV_DIRFT keyword not identified, the value is set to zero
        ...
        !! wavelengths are now already in the barycenter refeence system


        w2pix = (1._PR)*((-rvc_fin)/cc+1._PR)*w2pix


        !!! NOTE: new S2D are already corrected by blaze function, no need to include it in the analysis

        !file = trim(archive_files)//'/'//trim(file_night)//'/'//trim(file_rad)//'_S2D_BLAZE_'// &
        !     trim(fiber_sel)//'.fits'
        !call fits_check(file,sts_out)
        !if (sts_out.eq.0) then
        !   write(*,*) '     FILE '//trim(file)//' does not exist, no output file created for this exposure'
        !   cycle
        !end if
        !call fits_open_hdu_s2d(fits_lun,file,2,blaze)
        !call fits_close(fits_lun)
        !! blaze = 1._PR
...
end Program harpn_newDRS_input2pams
